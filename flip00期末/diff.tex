...
They have removed 5000 of the shot_made_flags (represented as missing values in the csv file). \DIFdelbegin \DIFdel{2cfd }\DIFdelend \DIFaddbegin \DIFadd{These are }\DIFaddend the test set shots for which we need submit a prediction. We are provided
...
KNeighbors Classifier and Logistic Regression.\DIFaddbegin \DIFadd{Use CV to find the best parameters of the algorithms,and then use these base model construct our ensemble model.
}\DIFaddend Finaly,make \DIFdelbegin \DIFdel{a final prediction,fit the model with whole data
}\DIFdelend \DIFaddbegin \DIFadd{the final prediction.
}\DIFaddend 
...
Use \DIFdelbegin \DIFdel{EDA }\DIFdelend \DIFaddbegin \DIFadd{exploratory data analysis }\DIFaddend to plot the distribution of the data,
